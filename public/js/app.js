...
const DEFAULT_LOCATION = { lat: 19.4326, lng: -99.1336, name: 'Ubicación por defecto' };

// Obtener ubicación inicial desde window.initialLocation (inyectada por EJS)
let currentLocation;
if (typeof window !== 'undefined' && window.initialLocation) {
    currentLocation = {
        lat: Number(window.initialLocation.lat) || DEFAULT_LOCATION.lat,
        lng: Number(window.initialLocation.lng) || DEFAULT_LOCATION.lng,
        name: window.initialLocation.name || DEFAULT_LOCATION.name
    };
} else {
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (clearMarkersBtn) clearMarkersBtn.addEventListener('click', clearAllMarkers);
...
function initGoogleMap() {
    ...
    googleMap.addListener('zoom_changed', () => {
        const el = document.getElementById('googleZoom');
        if (el) el.textContent = googleMap.getZoom();
    });
...
function initLeafletMap() {
    ...
    leafletMap.on('zoomend', () => {
        const el = document.getElementById('leafletZoom');
        if (el) el.textContent = leafletMap.getZoom();
    });
    ...
    navigator.geolocation.getCurrentPosition(
        async (position) => {
            ...
            try {
                ...
                if (data.success) {
                    currentLocation = { lat, lng, name: data.address };
                } else {
                    currentLocation = { lat, lng, name: `Coordenadas: ${lat.toFixed(6)}, ${lng.toFixed(6)}` };
                }
                ...
                if (document.getElementById('addressInput')) {
                    document.getElementById('addressInput').value = data && data.address ? data.address : '';
                }
...
async function handleMapClick(lat, lng) {
    try {
        ...
        const name = data && data.success ? data.address : `Coordenadas: ${lat.toFixed(6)}, ${lng.toFixed(6)}`;
...
function updateStats() {
    const gm = document.getElementById('googleMarkersCount');
    const lm = document.getElementById('leafletMarkersCount');
    const ts = document.getElementById('totalSearches');

    if (gm) gm.textContent = googleMarkers.length;
    if (lm) lm.textContent = leafletMarkers.length;
    if (ts) ts.textContent = searchCount;
}
...
function showNotification(message, type = 'info') {
    ...
    const iconName = type === 'success' ? 'check-circle' : (type === 'error' ? 'exclamation-triangle' : (type === 'warning' ? 'exclamation-triangle' : 'info-circle'));

    const notification = document.createElement('div');
    notification.className = `notification fixed top-4 right-4 ${colors[type] || colors.info} text-white px-6 py-3 rounded-lg shadow-lg z-50 transform transition-transform duration-300 translate-x-0`;
    notification.innerHTML = `
    <div class="flex items-center">
    <i class="fas fa-${iconName} mr-3"></i>
    <span>${message}</span>
    </div>
`;
...
function showLoading(show) {
    ...
    if (show) {
        ...
        <div class="flex items-center">
            <div class="animate-spin rounded-full h-8 w-8 border-b-2 border-google-blue mr-4"></div>
